...
final class MovieManager {
    ...
    func performRequest<T: Codable>(type: T.Type, query q: String, externalID exID: String, movieID mID: Int, movieIDSelection selection: movieIDSelection, movieURL: MovieURL, completion: @escaping (Result<T, Error>) -> Void){
        ...
        switch movieURL{
            ...
            case .discoverTv:
                url = URLAddress().discoverTvURL
            case .trendingTvs:
                url = URLAddress().urlTrendingTvs
            case .trendingMovies:
                url = URLAddress().urlTrendingMovies
            case .none:
                url = ""
        }
        if mID != 0{
            switch selection{
                ...
                case .movieImages:
                    url = "\(MovieConstants.baseURL)/\(MovieConstants.type)/\(String(mID))/images?\(MovieConstants.apiKey)&language=en-US"
                case .credits:
